...
`scan(imageData:orientation:)` validates the encoded image, reads EXIF orientation when the argument is `nil`, runs `VNRecognizeTextRequest`, converts recognized lines to `OCRToken`, and calls the injected `CardFieldClassifier`. The returned `AppleVisionScanResult` contains both `tokens` and structured `fields`.
